...
//                ReceiverAntenna rx = new ReceiverAntenna(1.0,Collections.singleton(S));
        ...
        for (Satellite imager : imagers) {
            ...
            ArrayList<Observation> planOutput = greedyPlannerFromFile(imager.getName());
...
//        plotResults(greedyPlannerFilePath);
    ...
    public static ArrayList<Observation> greedyPlannerFromFile(String satellite) {
        double currentTime = 0;
        ArrayList<Observation> observations = new ArrayList<>();
        ArrayList<TopocentricFrame> recentTFs = new ArrayList<>();
        double maxTorque = 4e-3; // Nm, BCT XACT 4e-3
        ...
        try (BufferedReader br = new BufferedReader(new FileReader("./src/test/resources/accesses_6h_5sat_rew.csv"))) { // CHANGE THIS FOR YOUR IMPLEMENTATION
            String line;
            while ((line = br.readLine()) != null) {
                String[] values = line.split(",");
                //System.out.println(values[0]);
                //System.out.println(satellite);
                if (Objects.equals(values[0], satellite)) {
                    csvRecords.add(Arrays.asList(values));
                }
            }
        }
        catch (Exception e) {
            System.out.println("Exception occurred in greedyplannerfromfile: "+e);
        }
        Frame earthFrame = FramesFactory.getITRF(IERSConventions.IERS_2003, true);
        BodyShape earthShape = new OneAxisEllipsoid(Constants.WGS84_EARTH_EQUATORIAL_RADIUS,
                Constants.WGS84_EARTH_FLATTENING, earthFrame);
        while(true) {
            ...
            for(List<String> record : csvRecords) {
                double riseTime = Double.parseDouble(record.get(3));
                double setTime = Double.parseDouble(record.get(4));
                GeodeticPoint gp = new GeodeticPoint(Double.parseDouble(record.get(1)),Double.parseDouble(record.get(2)),0.0);
                ...
                double newAngle = Double.parseDouble(record.get(5));
                double slewTorque = 4*Math.abs(newAngle-lastAngle)*0.05/Math.pow(Math.abs(currentTime-riseTime),2);
                double slewRate = Math.abs(newAngle-lastAngle)/Math.abs(currentTime-riseTime) * 180/Math.PI;
                double maxSlewRate = 1;
                ...
                double reward = Double.parseDouble(record.get(6));
                if(reward > maximum) {
                    bestTF = tf;
                    maximum = reward;
                    bestRiseTime = riseTime;
