...
//            System.out.println(Arrays.toString(mergedDownlinks.getRiseAndSetTimesList()));
        ...
        Map<Satellite,ArrayList<Double>> satGreedyRewardMap = new HashMap<>();
        Map<Satellite,ArrayList<Double>> satForwardRewardMap = new HashMap<>();
        for (Satellite imager : imagers) {
            satGreedyRewardMap.put(imager,new ArrayList<>());
            satForwardRewardMap.put(imager,new ArrayList<>());
            ...
            for (int k = 0; k < 24; k++) {
                ...
                double startTime = k*3600;
                double endTime = (k+1)*3600;
...
//                }
                ...
                for(Observation obs : planOutput) {
                    greedyReward += obs.getObservationReward();
                }

                satGreedyRewardMap.get(imager).add(greedyReward);
                double forwardReward = 0;
                for(Observation obs : smdpOutput) {
                    forwardReward += obs.getObservationReward();
                }
                satForwardRewardMap.get(imager).add(forwardReward);
            }
        }
        ArrayList<Double> greedyRewardList = new ArrayList<>();
        ArrayList<Double> forwardRewardList = new ArrayList<>();
        for (int i = 0; i < 24; i++) {
            greedyRewardList.add(0.0);
            forwardRewardList.add(0.0);
        }
        for (Satellite imager : satGreedyRewardMap.keySet()) {
            for (int i = 0; i < satGreedyRewardMap.get(imager).size(); i++) {
                greedyRewardList.set(i,greedyRewardList.get(i)+satGreedyRewardMap.get(imager).get(i));
            }
        }
        for (Satellite imager : satForwardRewardMap.keySet()) {
            for (int i = 0; i < satForwardRewardMap.get(imager).size(); i++) {
                forwardRewardList.set(i,forwardRewardList.get(i)+satForwardRewardMap.get(imager).get(i));
            }
        }
        System.out.println(greedyRewardList);
        System.out.println(forwardRewardList);
...
//            dataLines.stream()
//                    .map(XPlanner_test::convertToCSV)
//                    .forEach(pw::println);
//        }
    ...
    public static void processRain(Map<GeodeticPoint,Double> gps, String filepath) {
        try {
            ...
            for (GeodeticPoint gp : gps.keySet()) {
                ...
                Coordinate[] polygonPoints = {leftUpper,rightUpper,leftLower,rightLower,leftUpper};
